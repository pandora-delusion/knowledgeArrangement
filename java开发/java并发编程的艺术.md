...
+ wait 方法，是 Object 类的方法。调用对象的wait() 方法，会导致当前线程**放弃对象的锁（线程暂停执行）**，进入对象的等待池（等待队列），只有调用对象的notify() 方法（或#notifyAll()方法）时，才能唤醒等待池中的线程进入等锁池（同步队列），如果线程重新获得对象的锁就可以进入就绪状态。
...
- 同步块
  ...
  当计数器为0则可以成功获取，获取后将锁计数器设为1也就是加1。当
